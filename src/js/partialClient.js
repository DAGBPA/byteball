...
function initWallet() {
	...
	function setWalletNameAndColor(walletName) {
		...
		var color = root.config.colorFor ? root.config.colorFor[root.focusedClient.credentials.walletId] : '#FADA58';
		if(!color) color = '#FADA58';
	...
	function setWalletsInMenu() {
		...
		for (var key in root.walletClients) {
			...
			html += '<li onclick="wallet.selectWallet(\'' + walletId + '\')" id="w' + walletId + '" class="nav-item ' + (walletId === selectedWalletId ? 'selected' : '') + '">' +
				'<a class="oh"><div class="avatar-wallet " style="background-color: ' + (colors && colors[walletId] ? colors[walletId] : '#FADA58') + '">' + credentials.walletName.substr(0, 1) + ' </div>' +
