...
PROJECT="$BUILDDIR/../../dagpizzabuilds/project-$1-tn"
...
if [ ! -d $PROJECT ]; then
	...
	cordova create ../../dagpizzabuilds/project-$1-tn org.dagpizza.wallet.test DAGPizza
	checkOK
...
#  checkOK
	...
	cordova plugin add cordova-plugin-customurlscheme --variable URL_SCHEME=dagpizza-tn
	checkOK
...
#	checkOK
	...
	cordova plugin add https://github.com/DAGBPA/cordova-sqlite-plugin.git
	checkOK
...
if $DBGJS
then
	echo "${OpenColor}${Green}* Generating dagpizza bundle (debug js)...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova
	checkOK
else
	echo "${OpenColor}${Green}* Generating dagpizza bundle...${CloseColor}"
	cd $BUILDDIR/..
	grunt cordova-prod
	checkOK
fi
...
cp node_modules/dag-pizza-dough/initial.dagpizza.sqlite $PROJECT/www
cp node_modules/dag-pizza-dough/initial.dagpizza-light.sqlite $PROJECT/www
checkOK
...
if [ $CURRENT_OS == "ANDROID" ]; then
	...
	cat $BUILDDIR/android/android.css >> $PROJECT/www/css/dagpizza.css
...
#  checkOK
#
#  cp ios/DAGPizza-Info.plist $PROJECT/platforms/ios/DAGPizza-Info.plist
#  checkOK
#
#  mkdir -p $PROJECT/platforms/ios/DAGPizza/Resources/icons
#  checkOK
#
#  mkdir -p $PROJECT/platforms/ios/DAGPizza/Resources/splash
#  checkOK
#
#  cp -R ios/icons/* $PROJECT/platforms/ios/DAGPizza/Resources/icons
#  checkOK
#
#  cp -R ios/splash/* $PROJECT/platforms/ios/DAGPizza/Resources/splash
#  checkOK
fi
